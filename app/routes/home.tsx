import { ChevronLeft, ChevronRight, Newspaper, RefreshCw, Search } from "lucide-react";
import {
  Link,
  redirect,
  useActionData,
...
export async function loader({ request }: Route.LoaderArgs) {
  ...
  const [articlesResult, tags] = await Promise.all([
    getArticles({ page, limit: ARTICLES_PER_PAGE, status: "visible", search }),
    getTags(),
  ]);
    ...
    <div className="p-8">
      ...
      <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          <Newspaper className="h-5 w-5 text-sidebar-accent" />
